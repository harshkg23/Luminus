...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    es.onmessage = (ev) => {
      try {
        ...
        /* detect transitions → append to activity */
        ...
        for (const [step, status] of Object.entries(snap.steps)) {
          ...
          if (prev !== status) {
            if (status === "completed") {
              entries.push({
                title: `${STEP_LABELS[step] ?? step} completed`,
                time: now, err: false,
                desc: `Agent step "${STEP_LABELS[step] ?? step}" finished successfully.`,
              });
            } else if (status === "failed") {
              entries.push({
                title: `${STEP_LABELS[step] ?? step} failed`,
                time: now, err: true,
                desc: `Agent step "${STEP_LABELS[step] ?? step}" encountered an error.`,
              });
            } else if (status === "running") {
              entries.push({
                title: `${STEP_LABELS[step] ?? step} started`,
                time: now, err: false,
                desc: `Agent step "${STEP_LABELS[step] ?? step}" is now running.`,
              });
            }
          }
        }
        ...
        if (entries.length > 0) {
          setActivity((prev) => [...entries, ...prev].slice(0, 20));
        }
      } catch { /* ignore */ }
    };
  ...
  /* derive running / completed counts */
  ...
  return (
    <>
      ...
      <div className="p-7 space-y-8 max-w-[1600px]">
          ...
          {[
            {
              ...
              dc: runningCount > 0 ? "text-accent" : "text-fg-3",
            },
            {
              label: "Pipeline Status",
              value: sseOk ? "Live" : "—",
              unit: "",
              icon: sseOk ? "wifi" : "wifi_off",
              delta: sseOk ? "SSE stream connected" : "Waiting for connection",
              dc: sseOk ? "text-pos" : "text-fg-3",
            },
            {
              label: "Log Events",
              value: String(snapshot.logs.length),
              unit: "entries",
              icon: "article",
              delta: snapshot.logs.length > 0
                ? `Last: ${formatTs(snapshot.logs[snapshot.logs.length - 1].ts)}`
                : "No events yet",
              dc: "text-fg-3",
            },
        ...
        {/* Agent Squad — live status from snapshot */}
        <section>
              ...
              return (
                <div key={a.id} className={`glass-panel p-5 rounded-xl border-l-2 ${col.border}`}>
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <span className={`px-2 py-0.5 rounded font-mono text-[9px] uppercase tracking-widest ${col.bg}`}>
                      {status}
                    </span>
                  </div>
        ...
        {/* Live Stream terminal + Recent Activity */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-7">

          {/* Terminal — live pipeline logs via SSE */}
            ...
            <div className="flex items-center justify-between bg-[var(--bg-elevated)] px-4 py-2 rounded-t-xl border border-[var(--bd)] border-b-0">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-[9px] text-fg-3 uppercase tracking-widest ml-2">
                  Live Stream · Pipeline Agent Logs
                </span>
              </div>
                ...
                <div className="space-y-2">
                  {snapshot.logs.map((l: LogEntry, i: number) => (
                    <div key={`${l.ts}-${i}`} className="flex gap-4">
                      ...
                      <span className={
                        l.level === "error"   ? "text-neg"  :
                        l.level === "success" ? "text-pos"  :
                        l.level === "warn"    ? "text-warn" : "text-fg-2"
                      }>{l.message}</span>
                    </div>
                  ))}
          ...
          {/* Recent Activity — live from pipeline transitions */}
